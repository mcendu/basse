...
size_t
base64_decoder_size_nehalem (void)
{
  return BASE64_MAX_DECODER_SIZE;
}

void
base64_decoder_init_nehalem (struct base64_decoder *restrict ctx)
{
...
bool
base64_decode_nehalem (struct base64_decoder *restrict ctx,
                       ...
                       char *restrict out, size_t *restrict outlen)
{
  if (ctx)
      ...
      while (input_remaining > 0)
        {
          ...
          size_t block_size = base64_denewline_nehalem (
          ...
          // decode
          switch (base64_decode_main_nehalem (ctx->buffer, &block_processed,
                                              out, &block_outputted))
            {
  ...
  else
    {
      ...
      int result = base64_decode_main_nehalem (in, &remaining, out, outlen);
